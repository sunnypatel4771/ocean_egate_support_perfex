                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <?php if (isset($expense) && $expense->attachment !== '') { ?>
                            <div class="row">
                                <div class="col-md-10">
                                    <i class="<?php echo get_mime_class($expense->filetype); ?>"></i> <a href="<?php echo site_url('download/file/expense/' . $expense->expenseid); ?>"><?php echo e($expense->attachment); ?></a>
                                </div>
                        ...
                        <i class="fa-regular fa-circle-question pull-left tw-mt-0.5 tw-mr-1" data-toggle="tooltip" data-title="<?php echo _l('expense_name_help'); ?> - <?php echo e(_l('expense_field_billable_help', _l('expense_name'))); ?>"></i>
                        <?php $value = (isset($expense) ? $expense->expense_name : ''); ?>
                        <?php echo render_input('expense_name', 'expense_name', $value); ?>
                        <i class="fa-regular fa-circle-question pull-left tw-mt-0.5 tw-mr-1" data-toggle="tooltip" data-title="<?php echo e(_l('expense_field_billable_help', _l('expense_add_edit_note'))); ?>"></i>
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group select-placeholder">
                                    ...
                                    <select class="selectpicker display-block" data-width="100%" name="tax" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        ...
                                        <?php foreach ($taxes as $tax) {
                                            ...
                                            <option value="<?php echo e($tax['id']); ?>" <?php echo e($selected); ?> data-percent="<?php echo e($tax['taxrate']); ?>" data-subtext="<?php echo e($tax['name']); ?>">
                            ...
                            <div class="col-md-6">
                                <div class="form-group select-placeholder">
                                    ...
                                    <select class="selectpicker display-block" data-width="100%" name="tax2" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" <?php if (!isset($expense) || isset($expense) && $expense->tax == 0) {
                                                                                                                                                                                            echo 'disabled';
                                                                                                                                                                                        } ?>>
                                        <option value=""><?php echo _l('no_tax'); ?></option>
                                        <?php foreach ($taxes as $tax) {
                        ...
                        <div class="form-group select-placeholder" <?php if (isset($expense) && !empty($expense->recurring_from)) { ?> data-toggle="tooltip" data-title="<?php echo _l('create_recurring_from_child_error_message', [_l('expense_lowercase'), _l('expense_lowercase'), _l('expense_lowercase')]); ?>" <?php } ?>>
                            ...
                            <select name="repeat_every" id="repeat_every" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" <?php if (isset($expense) && !empty($expense->recurring_from)) { ?> disabled <?php } ?>>
                                <option value=""></option>
                        ...
                        <div class="checkbox checkbox-danger">
                                <?php
                                $checkPayeeValue = isset($expense->issue_payment_check) && $expense->issue_payment_check == 1 ? '1' : '0';
                                $checked = isset($expense->issue_payment_check) && $expense->issue_payment_check == 1 ? 'checked' : '';
                                ?>

                                <input type="hidden" name="issue_payment_check" value="0">
                                <input type="checkbox" name="issue_payment_check" id="issue_payment_check" value="1" <?php echo $checked; ?>>
                                <label for="issue_payment_check"><?php echo _l('issue_payment_check'); ?></label>
                            </div>
                            <div id="issue_payee_div" style="display: none; border: 1px solid; padding: 15px;">
                                <div class="checkbox checkbox-danger mb-3">
                                    <?php
                                    $checkPayeeValue = isset($expense->check_payee) && $expense->check_payee == 1 ? '1' : '0';
                                    $checked = isset($expense->check_payee) && $expense->check_payee == 1 ? 'checked' : '';
                                    ?>
                                    <input type="hidden" name="check_payee" value="0">
                                    <input type="checkbox" name="check_payee" id="check_payee" value="1" <?php echo $checked; ?>>
                                    <label for="check_payee"><?php echo _l('check_payee'); ?></label>
                                </div>
                                <div id="payee_div" style="display: none;" class="mb-3">
                                    <?php $vendor_payee = isset($expense) ? $expense->vendor_payee : ''; ?>
                                    <?php echo render_input('vendor_payee', '', $vendor_payee); ?>
                                </div>

                                <?php 
                                 $get_check_history_data = get_check_history_data_by_expensive($expense->id);
                                ?>
                                <?php 
                                if($get_check_history_data->bank != '' && $get_check_history_data->check_date != '' && $get_check_history_data->check_no != '') {
                                ?>
                                    <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <?php
                                        $all_bank = get_bank_details();
                                        $bank = isset($get_check_history_data) ? $get_check_history_data->bank : '';
                                        ?>
                                        <?php echo render_select('bank', $all_bank, ['id', 'bank_name'], 'bank', $bank, ['data-actions-box' => true], [], '', '', true); ?>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <?php $check_no = isset($get_check_history_data) ? $get_check_history_data->check_no : ''; ?>
                                        <?php echo render_input('check_no', 'check_no', $check_no,'',array('readonly' => true)); ?>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <?php $check_date = isset($get_check_history_data) ? $get_check_history_data->check_date : ''; ?>
                                        <?php echo render_date_input('check_date', 'check_date', $check_date,array('readonly' => true)); ?>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <?php $check_note = isset($get_check_history_data) ? $get_check_history_data->check_note : ''; ?>
                                         <?php echo render_input('check_note', 'check_note', $check_note,'',array('readonly' => true)); ?>
                                    </div>
                                    <!-- <div class="col-md-3 d-flex align-items-end">
                                        <label>&nbsp;&nbsp;&nbsp;</label>
                                        <a id="printCheckBtn" target="_blank" class="btn btn-primary btn-icon"><?php //echo _l('print_check'); ?></a>
                                    </div> -->
                                    </div>
                                <?php } ?>
                            </div>
                        <div>
                            ...
                            ?>
                            <div class="checkbox checkbox-primary billable_recurring_options <?php echo e($hide_invoice_recurring_options); ?>">
                                ...
                                <label for="create_invoice_billable"><i class="fa-regular fa-circle-question" data-toggle="tooltip" title="<?php echo _l('expense_recurring_autocreate_invoice_tooltip'); ?>"></i>
...
<script>
    ...
    $(document).ready(function() {
        ...
        //     return {
        //         check_payee: $('#check_payee').is(':checked') ? $('#check_payee').val() : '0',
        //         issue_payment_check: $('#issue_payment_check').is(':checked') ? $('#issue_payment_check').val() : '0',
        //         vendor_payee: $('#vendor_payee').val(),
        //         bank: $('#bank').val(),
        ...
        // $('#printCheckBtn').click(function() {
        //     const expense = getExpenseData();

        //     var errorMessages = [];

        //     if (expense.bank == '') {
        //         errorMessages.push('Bank field is required');
        //     }

        //     if (expense.check_no == '') {
        //         errorMessages.push('Check No field is required');
        //     }

        //     if (expense.check_date == '') {
        //         errorMessages.push('Check Date field is required');
        //     }

        //     if (errorMessages.length > 0) {
        //         alert_float('warning', errorMessages.join('<br>'));
        //         return;
        //     }
        ...
        //             .join('&');
        //     }
        //     const queryString = serialize(expense);
        //     const baseUrl = admin_url + 'checkwriter/printcheck/' + '<?php //echo $expense->id; ?>';
        //     const url = `${baseUrl}?${queryString}`;
        //     $(this).attr('href', url).text('Print Check');
        // });
    });


    $(function() {
        function togglePayeeDiv() {
            if ($('#check_payee').is(':checked')) {
                $('#payee_div').show();
            } else {
                $('#payee_div').hide();
            }
        ...
        $('#check_payee').change(function() {
            togglePayeeDiv();
        });

        function toggleipayment_checkDiv() {
            
            if ($('#issue_payment_check').is(':checked')) {
                $('#issue_payee_div').show();
            } else {
                $('#issue_payee_div').hide();
            }
        }

        toggleipayment_checkDiv();

        $('#issue_payment_check').change(function() {

(function ($) {
  ...
  var ProposalServerParams = {
    ...
    vessel_name: "[name='vessel_name[]']",
    manning_agency: "[name='manning_agency[]']",
    follow_up_filter: "[name='follow_up_filter']",
  };

  var table_rec_candidate = $(".table-table_rec_candidate");
  initDataTable(
    ".table-table_rec_candidate",
    admin_url + "recruitment/table_candidates",
    [5],
    [5],
    ProposalServerParams,
    [3, "desc"]
  );

  // var hidden_columns = [0, 2, 3];
  // $(".table-table_rec_candidate")
  //   .DataTable()
  ...
  $.each(ProposalServerParams, function (i, obj) {
    $("select" + obj).on("change", function () {
      var nameAttribute = obj.match(/\[name='([^']*)'\]/)[1];

      if (nameAttribute == "vessel_name[]") {
        table_rec_candidate.DataTable().order([2, "asc"]).draw();
      } else {
  ...
  $(document).on("click", "#follow_up_filter_btn", function () {
    var follow_up_filter = $("#follow_up_filter").val();
    if (follow_up_filter == 1) {
        $(this).html('Follow Up');
      $("#follow_up_filter").val(0).trigger("change");
    } else {
        $(this).html('View All');
      $("#follow_up_filter").val(1).trigger("change");
    }
  });

  $(document).on("change", "#follow_up_filter", function () {
    var val = $(this).val();
    var candidate_table = $(".table-table_rec_candidate");
    dt_custom_view(val, candidate_table);
  });
...
function candidate_profile_kanban() {
  "use strict";
  recruitment_init_kanban(
    "recruitment/kanban",
    candidate_profile_kanban_update,
    ".tasks-status",
    265,
    360
  );
...
function recruitment_init_kanban(
  url,
  callbackUpdate,
  connect_with,
  column_px,
...
function candidate_kanban_load_more(
  status_id,
  e,
  url,
  column_px,
  ...
  var total_pages = $('[data-col-status-id="' + status_id + '"]').data(
    "total-pages"
  );
  if (page <= total_pages) {
    var sort_type = $('input[name="sort_type"]');
...
function candidate_change_status(status, task_id, url) {
  ...
  url =
    typeof url == "undefined"
      ? "recruitment/candidate_change_status/" + status + "/" + task_id
      : url;
...
function print_candidate_bulk_actions() {
  ...
  $("#table_commodity_list_print_candidate option:selected")
    .prop("selected", false)
    .change();
  $(
    "table_commodity_list_print_candidate select[id='item_select_print_candidate']"

<script>
	...
	$(function() {
		...
		$(".edit_travel_info_detail").on("click", function() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					$("input[name='travel_id']").val(data.tr_id);
		...
		$(".passport_info_detail_delete").on("click", function() {
			var confirm = window.confirm("Are You Sure you Want to Delete?");
			if (confirm) {
				var id = $(this).attr("data-id");
				$.ajax({
	...
	$(document).ready(function() {
		...
		function filterTable(filterSelector, columnIdx) {
			...
			updateTotalBoardingPeriod(selectedNames);
		}

		function updateTotalBoardingPeriod(selectedNames = '') {
			...
			if (selectedNames.length == 1 && selectedNames[0] == 'BULK VESSELS') {
		...
		function set_vsl_carrer() {
			$("#vessel_filter_board").val('2').trigger('change').selectpicker('refresh');
			filterTable('#vessel_filter_board', 4);
	...
	$(document).ready(function() {
		...
		$('#vessel_filter_other,#vessel_filter_board_other, #rank_filter_other, #on_other_filter, #duty_filter_other').selectpicker();

		updateTotalBoardingPeriodOther();

		$('#vessel_filter_other,#vessel_filter_board_other, #rank_filter_other, #on_other_filter , #duty_filter_other').on('changed.bs.select', function() {
			filterTableOther();
		});
		...
		function filterTableOther() {
			var filters = [{
				...
				{
					selector: '#vessel_filter_board_other',
					columnIdx: 4
				},
				{
		...
		// $('#travel_document').DataTable().clear().destroy();
	});
</script>
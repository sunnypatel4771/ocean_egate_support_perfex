<script>
	...
	$(function() {
		...
		$(".edit_on_board_company_detail").on("click", function() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					$("select[name='employer']").selectpicker('destroy');
					$("select[name='employer']").val(data.employer);
					$("select[name='employer']").selectpicker('refresh');
		...
		$('#addOnBoardCompanyModal').on('shown.bs.modal', function() {
			$("select[name='employer']").selectpicker('destroy');
			$("select[name='employer']").selectpicker('refresh');
		});
	...
	$(document).ready(function() {
		...
		function updateTotalBoardingPeriodOther(selectedNames = '', dutyValue = '', vslValue = '') {
			...
			if (other_duty_rank_label) {
				...
				var tableother = $("#vessel_table_other").DataTable();
				tableother.rows({
					search: 'applied'
				}).every(function() {
					var rowData = this.data();
					if (rowData[2] === other_duty_rank_label) {
				...
				$('.main_other_rank_career').text(years + 'Y ' + remain_months + 'M ' + remain_days + 'D');
			}
	...
	$(document).on('click', ".delete_rec_contract", function() {
		var id = $(this).data('id');
		$.ajax({
			url: '<?php echo admin_url('recruitment/delete_rec_contract'); ?>',
			type: 'POST',
			data: {
				id: id
			},
	...
	$(document).on('click', ".edit_rec_contract", function() {
		var id = $(this).data('id');
		$.ajax({
			url: '<?php echo admin_url('recruitment/edit_rec_contract'); ?>',
			type: 'POST',
			data: {
				id: id
			},
			success: function(response) {
				...
				if (data.status == 1) {
					...
					$("#rec_contract_id").val(contract.id);
					$("#contract_name").val(contract.contract_name);
					$("#contract_template").val(contract.contract_template).change();
					$("#con_start_date").val(contract.con_start_date);
					$("#con_finish_date").val(contract.con_finish_date);

					$("select[name='contract_duration']").selectpicker("destroy");
					$("select[name='contract_duration']").val(contract.contract_duration);
					$("select[name='contract_duration']").selectpicker("refresh");
					// $("#template_content").val(contract.template_content);

					$("#con_signed_contract_edit_iamge").html(data.rec_signed_contract);
					var defult_contract_template_id = $("#defult_contract_template_id").val();
					if (contract.contract_template == defult_contract_template_id) {
						$('.template_edit_error').html('<p style="color: red; text-align: center;">For this Template the changes of the template design can not saved</p>');
					} else {
						$('.template_edit_error').html('');
					}
					...
					setTimeout(() => {
						tinymce.get('template_content').setContent(contract.template_content);
					}, 1000);
	...
	initDataTable('.table-rec_contract_list',
		admin_url + 'recruitment/rec_contract_list_table/' + candidate_id, undefined, undefined,
		'undefined', [0, 'desc']);
</script>
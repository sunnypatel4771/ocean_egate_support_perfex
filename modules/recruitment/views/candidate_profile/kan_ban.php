  ...
  <ul class="kan-ban-col tasks-kanban" data-col-status-id="<?php echo html_entity_decode($status['id']); ?>" data-total-pages="<?php echo html_entity_decode($total_pages); ?>">
      <li class="kan-ban-col-wrapper">
          <div class="border-right panel_s">
              <div class="panel-heading-bg" style="background-color: <?php echo $status['color']; ?>" data-status-id="<?php echo html_entity_decode($status['id']); ?>">
                  <div class="kan-ban-step-indicator<?php if ($status['id'] == 10) {
                                                          echo ' kan-ban-step-indicator-full';
                                                        } ?>"></div>

                  <span class="heading span_heading_color"><?php echo $status['status']; ?></span>
              ...
              <div class="kan-ban-content-wrapper kan-ban-wrapper-min-height">
                  <div class="kan-ban-content">
                      <ul class="status tasks-status sortable relative" data-task-status-id="<?php echo html_entity_decode($status['id']); ?>">
                          <?php
                          // Fetch candidates for this status
                          ...
                          foreach ($candidates as $candidate) {
                              if ($candidate['status'] == $status['id']) {
                                  $this->load->view('recruitment/candidate_profile/_kan_ban_card', ['candidate' => $candidate, 'status' => $status['id']]);
                              }
                          } ?>
                          <?php if ($total_candidates > 0) { ?>
                              <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo html_entity_decode($status['id']); ?>">
                                  <a href="#" class="btn btn-default btn-block<?php if ($total_pages <= 1) {
                                                                                  echo ' disabled';
                                                                                } ?>" data-page="1" onclick="candidate_kanban_load_more(<?php echo html_entity_decode($status['id']); ?>,this,'recruitment/recruitment_kanban_load_more',265,360); return false;"><?php echo _l('load_more'); ?></a>
                              </li>
                          <?php } ?>
                          <li class="text-center not-sortable mtop30 kanban-empty<?php if ($total_candidates > 0) {
                                                                                  echo ' hide';
                                                                                } ?>">
                              <h4>
                                  <i class="fa fa-circle-o-notch" aria-hidden="true"></i><br /><br />
                                  <?php echo _l('no_candidate_found'); ?>
                              </h4>
                          </li>
                      </ul>
                  </div>

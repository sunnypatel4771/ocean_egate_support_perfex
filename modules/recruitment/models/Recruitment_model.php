...
class Recruitment_model extends App_Model
{
	...
	public function tab_display_setting_contract($data)
	{
		$val = $data['input_name_status'] == 'true' ? 1 : 0;
		$this->db->where('name', $data['input_name']);
		$option_data = $this->db->get(db_prefix() . 'options')->result_array();
		if (!empty($option_data)) {
			...
			$this->db->insert(db_prefix() . 'options', $insert_data);
		}
	...
	public function get_manning_agency_list()
	{
	...
	{
		if (!empty($data)) {
			$this->db->insert(db_prefix() . 'rec_contract_template_for_crew', $data);
			if ($this->db->affected_rows() > 0) {
				return true;
			}
			return false;
	...
	public function delete_contract_template($id)
	{
		if (is_numeric($id) && $id > 0) {
			$this->db->where('id', $id);
			$this->db->delete(db_prefix() . 'rec_contract_template_for_crew');
			if ($this->db->affected_rows() > 0) {
				return true;
			}
		}
	...
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'rec_contract_template_for_crew', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
	...
	public function get_candidate_detail_by_field($field_name, $candidate_id)
	{
		if ($field_name != '' && $candidate_id != '') {
			$this->db->where(db_prefix() . 'rec_candidate.id', $candidate_id);

			if ($field_name == 'manning_agency') {
				$this->db->select(db_prefix() . 'manning_agency.agency_name AS manning_agency');
				$this->db->join(db_prefix() . 'manning_agency', db_prefix() . 'manning_agency.id =' . db_prefix() . 'rec_candidate.manning_agency');
			} elseif ($field_name == 'address') {
				$this->db->select("TRIM(TRAILING ',' FROM CONCAT(
                    COALESCE(" . db_prefix() . "rec_candidate.resident, ''), ', ',
                    COALESCE(" . db_prefix() . "cities_new.name, ''), ', ',
                    COALESCE(" . db_prefix() . "states_new.name, ''), ', ',
                    COALESCE(" . db_prefix() . "countries_new.name, '')
                )) AS address", false);
				$this->db->join(db_prefix() . 'cities_new', db_prefix() . 'cities_new.id = ' . db_prefix() . 'rec_candidate.city', 'left');
				$this->db->join(db_prefix() . 'states_new', db_prefix() . 'states_new.id = ' . db_prefix() . 'rec_candidate.state', 'left');
				$this->db->join(db_prefix() . 'countries_new', db_prefix() . 'countries_new.id = ' . db_prefix() . 'rec_candidate.address_country', 'left');
			} elseif ($field_name == 'rank') {
				// $this->db->select($field_name);
				$this->db->select(db_prefix() . 'job_rank.rank_name AS rank');
				$this->db->join(db_prefix() . 'job_rank', db_prefix() . 'job_rank.id = ' . db_prefix() . 'rec_candidate.rank', 'left');
			} else {
				$this->db->select($field_name);
			}
			$candidate_detail =	$this->db->get(db_prefix() . 'rec_candidate')->result_array();
			return $candidate_detail;
		}
	}

	public function add_rec_contract($data)
	{
		if (!empty($data)) {
			// rec_contract
			if (!empty($data)) {
				$this->db->insert(db_prefix() . 'rec_contract', $data);
				if ($this->db->affected_rows() > 0) {
					return true;
				}
				return false;
	...
	public function get_rec_contract($candidate_id)
	{
		$this->db->select(db_prefix() . 'rec_contract.*, ' . db_prefix() . 'rec_contract_template_for_crew.contract_template_name');
		$this->db->from(db_prefix() . 'rec_contract');
		$this->db->join(db_prefix() . 'rec_contract_template_for_crew', db_prefix() . 'rec_contract_template_for_crew.id = ' . db_prefix() . 'rec_contract.contract_template');
		$this->db->where(db_prefix() . 'rec_contract.candidate_id', $candidate_id);
		$query = $this->db->get();
		$contract_template = $query->result_array();
		// $contract_template = $this->db->get(db_prefix() . 'rec_contract')->result_array();
		if ($contract_template) {
			return $contract_template;
		}
	...
	public function delete_rec_contract($id)
	{
		if (is_numeric($id) && $id > 0) {
			$this->db->where('id', $id);
			$this->db->delete(db_prefix() . 'rec_contract');
			if ($this->db->affected_rows() > 0) {
				return true;
			}
		}
	...
	public function get_rec_contract_by_id($id)
	{
		if (is_numeric($id) && $id > 0) {
			$this->db->where('id', $id);
			$contract =	$this->db->get(db_prefix() . 'rec_contract')->result_array();
			if (!empty($contract)) {
				return $contract;
			}
		}
	...
	// 
	public function update_rec_contract($data, $id)
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'rec_contract', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return true;
	...
	// 	added for add defult contract in crew
	public function get_all_candidate_ids()
	{
		$this->db->select('id');
		$candidates = $this->db->get(db_prefix() . 'rec_candidate')->result_array();
		if (!empty($candidates)) {
	...
	public function check_contract_is_there_for_crew($candidate_id, $contract_template_id)
	{
		$this->db->where('candidate_id', $candidate_id);
		$this->db->where('contract_template', $contract_template_id);
		$query = $this->db->get(db_prefix() . 'rec_contract');

		return ($query->num_rows() > 0) ? false : true;
	}

	public function get_custom_filed_setting_principal_data($id = '')
	{
		if ($id != '') {
			$this->db->where('id', $id);
			$data = $this->db->get(db_prefix() . 'custom_filed_setting_principals')->row_array();
			if (!empty($data)) {
				return $data;
			}
		} else {
			$data = $this->db->get(db_prefix() . 'custom_filed_setting_principals')->result_array();
			if (!empty($data)) {
				return $data;
			}
		}
	...
	// add_custom_filed_setting_principals
	public function add_custom_filed_setting_principals($data)
	{
		if (!empty($data)) {
			$this->db->insert(db_prefix() . 'custom_filed_setting_principals', $data);
			if ($this->db->affected_rows() > 0) {
				return true;
			}
			return false;
	...
	// update_custom_filed_setting_principals
	public function update_custom_filed_setting_principals($data, $id)
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'custom_filed_setting_principals', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
	...
	public function delete_custom_filed_setting_principals($id)
	{
		if (is_numeric($id) && $id > 0) {
			$this->db->where('id', $id);
			$this->db->delete(db_prefix() . 'custom_filed_setting_principals');
			if ($this->db->affected_rows() > 0) {
				return true;
			}
		}
	...
	public function add_custom_filed_setting_salary_scale($data)
	{
		if (!empty($data)) {
			$this->db->insert(db_prefix() . 'custom_filed_setting_salary_scale', $data);
			if ($this->db->affected_rows() > 0) {
				return true;
			}
			return false;
	...
	public function update_custom_filed_setting_salary_scale($data, $id)
	{
		$this->db->where('id', $id);
		$this->db->update(db_prefix() . 'custom_filed_setting_salary_scale', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		}
		return false;
	...
	public function get_custom_filed_setting_salary_scale_data($id = '')
	{
		if ($id != '') {
			$this->db->where('id', $id);
			$data = $this->db->get(db_prefix() . 'custom_filed_setting_salary_scale')->row_array();
			if (!empty($data)) {
				return $data;
			}
		} else {
			$data = $this->db->get(db_prefix() . 'custom_filed_setting_salary_scale')->result_array();
			if (!empty($data)) {
				return $data;
			}
		}
	...
	public function delete_custom_filed_setting_salary_scale($id)
	{
		if (is_numeric($id) && $id > 0) {
			$this->db->where('id', $id);
			$this->db->delete(db_prefix() . 'custom_filed_setting_salary_scale');
			if ($this->db->affected_rows() > 0) {
				return true;
			}
		}
